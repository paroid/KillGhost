...
class PHPWebSocket
{
    ...
    /*
        $this->wsClients[ integer ClientID ] = array(
			...
			12 => int  //us  Last message Sending Time?			//custom
        )
    ...
    function wsAddClient($socket, $clientIP) {
        ...
        $this->wsClients[$clientID] = array($socket, '', self::WS_READY_STATE_CONNECTING, time(), false, 0, $clientIP, false, 0, '', 0, 0,floor(microtime(true)*1000));
    ...
    function wsSendClientMessage($clientID, $opcode, $message) {
        ...
        if ($this->wsClients[$clientID][2] == self::WS_READY_STATE_CLOSING || $this->wsClients[$clientID][2] == self::WS_READY_STATE_CLOSED){ echo "Error: Client CLosed!\n"; return true;}

		//check the Ready 
		while(floor(microtime(true)*1000)-$this->wsClients[$clientID][12]<500){
			usleep(10000);
		}
		...
		//set the Ready flag
		$this->wsClients[$clientID][12]=floor(microtime(true)*1000);
        return true;
    }
